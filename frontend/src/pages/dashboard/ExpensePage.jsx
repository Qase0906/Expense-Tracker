...
const ExpensePage = () => {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const response = await api.get(`/expenses?type=${type}&date=${date}&page=${page}&limit=${limit}`);      
  ...
  const expense = data?.paginatedExpense;
  ...
  return (
    <div className="mt-20 w-full ">
      ...
      <div className="flex gap-2 m-4">
        <button onClick={() => setFilters(f => ({ ...f, type: "expense" }))}
